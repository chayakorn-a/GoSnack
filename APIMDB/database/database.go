package database

import (
	"database/sql"
	"fmt"
	"log"
	"os"
	//"strconv"
	"APIMDB/apigee"
	_ "github.com/lib/pq"
)

type Todo struct {
	ID     string `json:id`
	Title  string `json:title`
	Status string `json:status`
}

func GetSqlDB() *sql.DB {
	url := os.Getenv("DATABASE_URL")
	fmt.Println(url)
	db, err := sql.Open("postgres", url)

	if err != nil {
		log.Fatal("Connect to database error", err)
	}
	return db
}

func CreateTodoTable(db *sql.DB) {
	createTb := `
	CREATE TABLE IF NOT EXISTS todos (
	id SERIAL PRIMARY KEY,
	title TEXT,
	status TEXT
	);
	`
	_, err := db.Exec(createTb)

	if err != nil {
		panic(err)
	} else {
		fmt.Println("create table success")
	}
}

func RemoveAllDataTodoTable(db *sql.DB){
	sqlStatement := `DELETE FROM todos;`

	_, err := db.Exec(sqlStatement)

	if err != nil {
		panic(err)
	} else {
		fmt.Println("delete table success")
	}
}

func InsertTodoTable(db *sql.DB, title string, status string) {
	row := db.QueryRow("INSERT INTO todos (title, status) values ($1, $2) RETURNING id", title, status)
	var id int
	err := row.Scan(&id)
	if err != nil {
		fmt.Println("can't scan id", err)
		return
	}
	fmt.Println("insert todo scccess id : ", id)
}

func SyncToDb() []Todo{
	var todos []Todo

	db := GetSqlDB()
	defer db.Close()

	// create DB if it does not exist
	CreateTodoTable(db)
	RemoveAllDataTodoTable(db)
	people := apigee.QueryKvm()

	for _, k := range people {
		//fmt.Println(k.Name,"\t", k.Value)
		InsertTodoTable(db, k.Name, k.Value)
	}

	return todos
}
/*
func QueryAllTodoTable() []Todo {
	db := GetSqlDB()
	defer db.Close()

	stmt, err := db.Prepare("SELECT id, title, status FROM todos")
	if err != nil {
		log.Fatal("can't prepare query all todos statement", err)
	}

	rows, err := stmt.Query()
	if err != nil {
		log.Fatal("can't query all todos", err)
	}
	var todos []Todo
	for rows.Next() {
		var id int
		var title, status string
		err := rows.Scan(&id, &title, &status)
		if err != nil {
			log.Fatal("can't scan row into variable", err)
		}
		todo := Todo{strconv.Itoa(id), title, status}
		todos = append(todos, todo)
		//fmt.Println(id, title, status)
	}
	fmt.Println(todos)
	fmt.Println("query all todos success")
	return todos
}*/